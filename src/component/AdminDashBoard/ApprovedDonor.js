...
export default function ApprovedDonor() {
  ...
  const  updateUser=async()=>{
    axios
    .patch("http://localhost:9006/api/AdminUser/v1/approvedDonor/"+updatebeneficiary.donorId)
    .then((response) => {
      if (response.status === 200) {
        window.alert("You have Successfully Approved Donor")
    ...
    return (
      sessionStorage.getItem("adminId")!==null ? (
        ...
        >
          ...
          <h4 style={{"color":"black"}}className="text-center container my-2 pt-5">Approve Donor</h4>
